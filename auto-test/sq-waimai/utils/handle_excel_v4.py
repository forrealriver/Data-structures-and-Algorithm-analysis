...
def is_json(inStr):
    try:
        json.loads(inStr)#json转换成字符串，如果报错就说明不是json
    except:
        return False
    return True
#----------------------------------------------------------------------
    # *colName解包,selectCase = ['all']缺省参数，默认值all
def get_excel_data(sheetName,caseName,*colName,excelDir=None,selectCase=['all']):
    excelDir = data_path+r'\Delivery_System_V1.5.xls'
    #创建空列表接收数据
    resList = []
    ...
    for one in workSheet.col_values(0):
        if caseName in one and one in selectList:
            ...
            for colIndx in colIndxList:
                ...
                if is_json(res):#判断单元格数据是否是json（双引号）
                    res = json.loads(res)#转化成字典（单引号）
                rowList.append(res)
            #保存所有符合条件行的内容
